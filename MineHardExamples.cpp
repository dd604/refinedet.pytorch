  ...
  *num_negs = 0;
  ...
  if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    ...
    ComputeLocLoss(loc_pred, loc_gt, *all_match_indices, num,
  	...
  	// 目前的是max，MultiBoxLoss.MAX_NEGATIVE，使用neg_pos_ratio.
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = match_indices.begin();
      ...
      for (int m = 0; m < match_indices[label].size(); ++m) {
        // 1.对于hardnegative来说这个函数始终返回true;非hard下统计的是全部的
      	// 2.目前不是hard情况下，hard下统计的是negative的。
        // 如果没有arm_conf_data，那么就不过滤，否则就用arm_conf进行过滤
      	// 则是处理negative，并且是很负的样本overlap小于neg_overlap的。
      	// 这里不是正样本的都当成了负样本'neg_overlap': 0.5
      ...
      if (mining_type == MultiBoxLossParameter_MiningType_MAX_NEGATIVE) {
      	// max情况下，统计num_pos，约束num_sel，已经用loss排过了序列
      	// 由于neg是没有loc_loss的，所以也就是只有conf_loss的loss的排序。
				int num_pos = 0;
				for (int m = 0; m < match_indices[label].size(); ++m) {
      	...
      	// hard的时候，挑选相应的个数即可，sample_size，正负样本一起hard的。
      ...
      for (int m = 0; m < match_indices[label].size(); ++m) {
        // 1.hard negative情况下，如果匹配到了，并且没有被select到，
        // 更改索引为-1，表示取消匹配，不考虑，这些本来是正样本，正样本也是只用hard的。
        // 否则原本就是负样本，并且被选择到了，那么假如到neg_indeces中去。
      	// 2.MAX_NEGATIVE的时候，num_matches的数目是不发生变化的。
        if (match_indices[label][m] > -1) {
          if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE &&
